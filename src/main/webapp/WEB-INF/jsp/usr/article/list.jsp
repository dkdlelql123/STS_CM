...
<div class="flex-grow">
  ...
  <div class="w-3/4 mx-auto">
    ...
    <div id="pagenation" class="btn-group justify-center mt-4 mb-6">
      ...
      <c:set var="startPage"
        value="${page - pageMenuArmLen > 0 ? page - pageMenuArmLen : 1}" />
      <c:set var="endPage"
        value="${page + pageMenuArmLen <= end ? page + pageMenuArmLen: end }" />

      <c:set var="baseUri" value="?boardId=${board.id}" />
      <c:set var="baseUri"
        value="${baseUri}&searchType=${param.searchType}" />
      <c:set var="baseUri"
        value="${baseUri}&searchKeyword=${param.searchKeyword}" />

      <c:if test="${startPage > 1}">
        <a class="btn btn-sm btn-outline border-gray-400"
      ...
      <c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
        <a
          class="btn btn-sm btn-outline border-gray-400 ${page == i ? 'btn-active' :''}"
      ...
      <c:if test="${endPage < end}">
        <c:if test="${endPage < end-1 }">
          <a
            class="btn btn-sm btn-disabled btn-outline border-gray-400 ">...</a>
        </c:if>
        <a class="btn btn-sm btn-outline border-gray-400 "
          href="${baseUri}&page=${end}">${end}</a>
      </c:if>
    </div>
